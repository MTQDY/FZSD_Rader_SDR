...
毕竟本人本质硬件红丸，也不要对本人的算法能力有过多期待。  
  ...
  *项目结构*：  
  本项目分为两个线程，分别是GRC线程与python线程。与原来不同的是，GRC线程是一个独立的程序——不同于开源中接收前端、解码统一靠python调度，本项目的接收前端受GRC调度，二者只靠一架很窄的“桥”，
PMT消息通信。GRC无法让数据停下来处理，所以只能将信息发给应用层慢慢处理，应用层处理完后发送回调，修改GRC接收链路的参数。所有的GRC线程程序都在grc_rx_chain文件夹中。  
