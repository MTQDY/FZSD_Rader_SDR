...
class ApplicationHandler:
    ...
    def on_packets(self, packets: list[dict], ts: float) -> None:
        ...
        if not self.args.quiet and packets:
            ...
            # 接入码距离分布: 诊断 PLL 是否漂移
            dists = [p.get("best_info_dist", 64) for p in packets]
            d0 = sum(1 for d in dists if d == 0)
            d1 = sum(1 for d in dists if d == 1)
            d2p = sum(1 for d in dists if d >= 2)
            emit_json({"kind": "diag_packets", "ts": now, "total": len(packets),
                       "jam": jam_n, "info": info_n, "valid": valid_n,
                       "best_access_dist": best_dist,
                       "dist_0": d0, "dist_1": d1, "dist_ge2": d2p,
    ...
    def _handle_info_packets(self, packets: list[dict], now: float) -> None:
        ...
        # 诊断: 统计本轮流水线各环节
        total_info = len(info_packets)
        appended = 0
        dropped_bad_len = 0
        diag_before = dict(self.stream.diag)
        ...
        frames = self.stream.extract_frames(ts=now)
        diag_after = self.stream.diag

        # 诊断: 输出重组流水线各环节计数增量
        if not self.args.quiet:
            emit_json({
                "kind": "diag_info_chain",
                "ts": now,
                "air_total": total_info,           # 收到的有效 INFO 空中包
                "air_appended": appended,          # 成功拼入重组的
                "air_dropped": dropped_bad_len,    # len1/len2 校验失败被丢弃
                "frames_extracted": len(frames),   # 重组出的完整裁判帧
                "diag_sof": diag_after["sof_found"] - diag_before["sof_found"],
                "diag_crc8_fail": diag_after["crc8_fail"] - diag_before["crc8_fail"],
                "diag_len_invalid": diag_after["len_invalid"] - diag_before["len_invalid"],
                "diag_incomplete": diag_after["incomplete"] - diag_before["incomplete"],
                "diag_crc16_fail": diag_after["crc16_fail"] - diag_before["crc16_fail"],
                "diag_frames_ok": diag_after["frames_ok"] - diag_before["frames_ok"],
            })
    ...
    def apply_pending_level_change(self) -> bool:
        ...
        # 接收增益随 profile 切换; 若命令行显式指定 --rx-gain-db 则全程以它为准
        self.state.rx_gain_db = (
            profile.rx_gain_db if self.args.rx_gain_db is None else self.args.rx_gain_db
        )
        ...
        if should_reconfigure:
            ...
            self.rx_chain.reconfigure_for_profile(
                ...
                rx_gain_db=self.state.rx_gain_db,
            )
...
def main() -> int:
    ...
    parser.add_argument("--rx-gain-db", type=float, default=None,
                        help="接收增益 (dB)。默认 None = 由 profile 的 rx_gain_db 按模式自动切换; "
                             "显式指定则该值全程固定 (手动模式)")
    parser.add_argument("--access-bit-errors", type=int, default=2)
    ...
    state = ReceiverState(
        ...
        rx_gain_db=(profile.rx_gain_db if args.rx_gain_db is None else args.rx_gain_db),
    )
    ...
    if args.record_wave:
        ...
        try:
            wave_recorder.start({
                ...
                "rx_gain_db": state.rx_gain_db, "sps": args.sps, "bt": args.bt,
    ...
    try:
        ...
        while True:
            ...
            if handler.last_status_emit == 0 or now - handler.last_status_emit >= args.status_interval:
                ...
                # 读取 SDR 接收功率 (dBFS)
                try:
                    state.last_buffer_power_dbfs = round(rx_chain.get_power_dbfs(), 2)
                except Exception:
                    pass
            ...
            if now - handler.last_packet_time > 5.0:
                ...
                # 仅在手动指定增益时透传, 否则重启后仍由 profile 决定
                if args.rx_gain_db is not None:
                    new_argv += ["--rx-gain-db", str(args.rx_gain_db)]
